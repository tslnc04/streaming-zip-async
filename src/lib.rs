...
impl FileHeader {
    fn write<W: Write>(&self, handle: &mut W, is_central: bool) -> Result<usize> {
        ...
        written += 2;
        written += self.data_descriptor.clone().unwrap_or_default().write(
            handle,
            false,
            false,
            self.is_zip64,
...
impl<W: Write> Archive<W> {
    ...
    pub fn add_file_from_path<R: AsRef<Path>, S: AsRef<Path>>(
        &mut self,
        path: R,
        src_path: S,
        compression: CompressionMode,
    ...
    pub fn add_dir_all<R: AsRef<Path>, S: AsRef<Path>>(
        &mut self,
        path: R,
        src_path: S,
        compression: CompressionMode,
    ...
    pub fn finish(mut self) -> Result<W> {
        ...
        if is_zip64 {
            ...
            self.inner
                .write_all(&(self.files.len() as u64).to_le_bytes())?; // Number of central directory records on this disk
            self.inner
                .write_all(&(self.files.len() as u64).to_le_bytes())?; // Total number of central directory records
            self.inner
                .write_all(&(central_directory_size as u64).to_le_bytes())?; // Size of central directory
            self.inner
                .write_all(&(central_directory_start as u64).to_le_bytes())?; // Offset of start of central directory
            ...
            if self.files.len() > (u16::MAX as usize) {
                ...
                self.inner
                    .write_all(&(self.files.len() as u16).to_le_bytes())?; // Number of central directory records on this disk
                self.inner
                    .write_all(&(self.files.len() as u16).to_le_bytes())?; // Total number of central directory records
            }
            ...
            self.inner
                .write_all(&(self.files.len() as u16).to_le_bytes())?; // Total number of central directory records
            self.inner
                .write_all(&(central_directory_size as u32).to_le_bytes())?; // Size of central directory
            self.inner
                .write_all(&(central_directory_start as u32).to_le_bytes())?; // Offset of start of central directory
            self.inner.write_all(&0u16.to_le_bytes())?; // Comment length
        }
